...
+ event_at_idx\ Это BRIN-индекс, эффективный для временных данных. Он ускоряет выборки по диапазонам времени.

+ event_actor_idx \ Позволяет быстро найти все действия конкретного пользователя, администратора или системного процесса.

+ user_session_user_id_idx \ Ускоряет выборку всех активных сессий пользователя (например, при выходе со всех устройств).

+ user_session_expires_at_idx \ Позволяет быстро очистить просроченные сессии, не просматривая всю таблицу.
